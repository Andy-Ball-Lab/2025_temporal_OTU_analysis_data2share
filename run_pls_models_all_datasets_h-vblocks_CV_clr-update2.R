...
# --- Patch for plsmod mixOmics bug ("incorrect number of dimensions") from Sep 25 onwards.
...
choose_random_windows <- function(df, outcome, assess, n_windows = 2, seed = NULL) {
  ...
  n <- nrow(df)
  starts <- 1:(n - assess + 1)  # set all possible starting positions, nrow(df) = number of samples

  selected <- list()
  used_idx <- integer()

  while (length(selected) < n_windows && length(starts) > 0) {
    pick <- sample(starts, 1)   # random start - chooses a random starting point from the start vector
    this_window <- pick:(pick + assess - 1)  # creates a window from the random starting point
    selected <- append(selected, list(this_window))
    used_idx <- c(used_idx, this_window)

    # prevent overlaps (embargo 1 either side)
    embargo_range <- (min(this_window) - assess):(max(this_window) + assess)  # any samples in length of 'assess'
    starts <- setdiff(starts, embargo_range)                                  # before or after the selected window is embargoed for the next window
  }
...
# contiguous validation window (time-ordered block)
# purges neighboring samples by excluding ±h around it
# trains on the remaining samples
# repeats this process reps times with different random start positions.
...
make_hv_block_rset <- function(df, assess = 12, h = 2, reps = 50) {
  ...
  starts <- 1:(n - assess + 1)  # creates all possible starting indices
  starts <- base::sample(starts, size = reps, replace = (reps > length(starts))) # Replace = TRUE if there are fewer possible starts than repetitions,
                                                                                 # it samples with replacement, windows can overlap if series is short
  splits <- lapply(seq_along(starts), function(i) {   # loop across all selected start indices
    s <- starts[i]   # Grabs the i-th starting index for the current validation block
    assess_idx  <- s:(s + assess - 1)

# Purge/embargo around assessment window (lower and upper bound of the embargo region)
    embargo_lo <- max(1, s - h)
    embargo_hi <- min(n, s + assess - 1 + h)
    analysis_idx <- setdiff(seq_len(n), embargo_lo:embargo_hi)

   rsample::make_splits(             # construct the rsplit object from the rsample package
      list(analysis = analysis_idx, assessment = assess_idx),
      data = df
    )
  })
    rsample::manual_rset(splits, ids = paste0("hv_", seq_along(splits)))  # name each split, this can be passed to tune_grid
}
